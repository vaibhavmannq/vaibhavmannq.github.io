...
| D7 | **GSAP (since 2026-09-22)** for text motion: SplitText and ScrambleText, with each chapter's timeline scrubbed by the scroll handover, not ScrollTrigger. Originally "No GSAP" (Web Animations API + CSS) | Motion; Anime.js v4 | GSAP became free in 2025, including SplitText; the owner asked for richer motion after the first full journey (§17 S33) |
...
| Lumenreach (dropped 2026-09-21) | none | The lantern city, ring gate, tower, river and lantern motes were dropped with the shore's structures (§3.4). Projects lives on Moonsink Shore (§17 S31) |
| Lastlight Isle (dropped 2026-09-21) | none | The island city, arches, bell tower and canals were dropped like Lumenreach; Contact lives on Moonsink Shore (§17 S32) |
...
interface Project {
  ...
  tools: string[]; summary: string; details: string[];   // details: the dialog's paragraphs
...
- `?moon=0.3`: pin the moon's phase instead of the one the scroll gives (§17 S40)
- `?bare`: hide the text layer, for rendering the stills and the project cover (`npm run capture`)
- `?webgl`: force the WebGL2 backend
- (`?hold` went with the black opening, §17 S42.)
...
| S30 | Moon dial removed (2026-09-14) | The owner removed the dial the same day. What they liked in the look demo was what it showed, that a visitor sees a different moon each day, not a control on the site. The dial's markup, styles, phase names and tests are gone; the site is back to the real nightly phase, set once per visit (§5.4b). The horizon streak fix from S29 stays. The owner then weighed keeping the moon changing daily against changing its phase with each page (region), and chose the daily real moon: every visitor sees tonight's phase, and the moon holds still while scrolling (S27). Offered and not chosen: a phase per region, tonight's moon then stepping onward per region, and deciding at Lumenreach |
...
| S38 | Frame pacing follows the display (2026-09-25, spec `2026-09-25-waxing-voyage-design.md` §4.1) | Found in a harsh review. The old cap drew when 14.7 ms had passed, so 75, 90 and 144 Hz screens rendered at 37.5, 45 and 48 fps, with frames 20–27 ms apart. The governor read that as slow and stepped to tier 0 within 7 s, however fast the GPU (simulated with the real code). `app/refresh.ts` now measures the refresh from the median animation-frame delta and renders every Nth vsync: 90 Hz → 45 fps, 144 Hz → 72, evenly spaced. The governor's thresholds became ratios of that interval: 1.2× down, 1.05× up, which is exactly S16's 20 / 17.5 ms at 60 Hz. Found while building: (1) during the half-second measurement the old cap still runs, so the governor must not judge those frames, or a 75/144 Hz screen steps down after every resize; (2) Chrome throttled an obscured window to 1 frame a second, the pacer measured "1 Hz" and kept it, so the pacer now measures again when five frames in a row disagree by over 30%, which also follows a phone's low-power mode or a 60/120 Hz setting change. A simulation of 60–165 Hz with the real pacer and governor shows no tier change and one frame interval per rate |
| S39 | The ray march starts at the highest surface it can meet (2026-09-25, §4.2) | Nearly all of the sea's cost was rays creeping down through empty air. `surfaceTop.ts` bounds every surface ahead of the camera: the highest wave (≈ 0.454), or the sand at the camera's own z, since the view never turns and the sand falls away ahead. Rays above it heading up are sky with no steps; others start where they cross it. The step grows 0.5 → 0.7 of the height gap. Measured on the Intel UHD laptop at 1280×720, DPR 1.5, fps old → new: tier 2 46.6 → 60 (capped), tier 3 31.6 → 58, tier 4 21 → 41.4. Old and new frames compared at four positions: same composition, wave structure kept, only the glitter pattern differs. Glints now fade in away from the camera instead of peaking at it. `?march=old` and `?glints=old` keep the old versions for the owner's review |
| S40 | The moon waxes with the voyage (2026-09-26, §4.3) | Owner proposal after the harsh review: a moon cycle through the journey instead of the daily real moon (S30). A half cycle, chosen from the storyboard: 0.08 → 0.25 → 0.375 → 0.5 (crescent, first quarter, gibbous, full), each reached where its chapter is fully shown (anchor + SHOWN_OFFSET) and eased between. A pure function of scroll (`journey/journeyMoon.ts`); reduced motion holds each chapter's phase. The voyage log names the phase the scene shows. `?moon=` still pins it for tests |
| S41 | The camera lands up the beach (2026-09-26, §4.4) | Owner pick ("move the camera up the beach"): the landing moved from 2.7 to 2.0 screens so the black sand is on screen for Projects and Contact. Path keys at 0, SHORE/3, 2·SHORE/3, SHORE, WALK_END and 1; yaw 0 and pitch −0.06 throughout, so the moon still never moves on screen |
| S42 | Opening E: the name at first paint (2026-09-26, §4.5) | Owner pick E. The black layer, the hold, the scroll lock and "Skip intro" are gone; the HTML is the first paint and the sea fades up under it. Measured: the name was readable 5.6 s in, now at first contentful paint (≈ 300 ms). A waking loop moves the text before the 3D loop starts; the page settles (header, rail, "Hello, voyager" scrambling into "I · Adrift") 1.2 s in, or when the scene is ready, and by 3 s at the latest (final review I1). Supersedes S37's ordering rule |
| S43 | Header, rail, focus, fonts, and the case study (2026-09-26, §4.6–4.8) | A header with "Email me", a rail of four moon glyphs, and a focus glide so nothing focused is ever invisible (it waits until the page is fully shown; Tab and modifier keys never stop a glide). All fonts are self-hosted (Fraunces 67 kB, Space Mono 16.5 kB). About is two paragraphs, which fixed the orphaned word SplitText kept producing. Owner pick F: the project card has a cover and the case study a picture, the hard part (smoothness) and a number (31.6 → 58 fps, tier 3 on an Intel UHD laptop), rendered from the real scene by `npm run capture`. Owner review on phone and laptop: "everything is perfect"; the case study now scrolls on a phone (`data-lenis-prevent`) and "See the work" has no shadow. The owner kept the new side of every review switch (`?pace`, `?march`, `?glints`, `?frame`); mouse-wheel snapping (`?snap=wheel`) was never picked, so it stays off and every switch is removed |
| S44 | One page at a time, however fast the scroll (2026-09-25, §4.12) | Owner report: a fast scroll showed "Projects", "Moonlit", the About text and "Hola Amigo" at once. Measured: 14 hard wheel notches left all four visible at 0.06–0.35 opacity for about a second, both ways, because S35 eased each page on its own. `createHandover` (sections.ts) keeps one shown page. When the scroll lands on another page, the shown one leaves in about 0.25 s (up and out, or sinking back when scrolling up), and only then does the next arrive. Pages flown past never appear, and a slow scroll matches S35 within 0.01. The same wheel run in the browser now shows at most one page, in Chromium, Firefox and WebKit |
| S45 | No box around the big headings (2026-09-25, §4.13) | Owner report: a faint rectangle around the name, "Projects" and "Hola Amigo", and the "g" of "Amigo" cut off. SplitText's line and letter masks clip at their own box. At a line-height of 0.9 a heading's glyphs overhang that box by about 19 px, and S37's 20 px halo reaches further still. The masks now clip with `clip-path: inset(-(0.2em + 20px))`, which leaves the layout unchanged; SplitText's inline `overflow: clip` is switched off after each split; lines travel 200% (letters 180%) so they are still hidden when they start to rise. A test compares each heading with and without any clipping: identical pixels |
| S46 | A window and full screen show the same composition (2026-09-25, §4.14) | Owner report: the text moved between a normal window and Fn+F11. The text was sized by the window's width and the scene by its height; measured at 1280×590 against 1280×720, the pages sat 6–13% of the height apart. `overlay/frame.ts` scales and shifts each page into the scene's 16:9 frame. Now within 0.5% in Chromium and Firefox, and 1.5% in WebKit, which lays out scaled text about 4 px taller at DPR 2. Found while building: WebKit could run boot before the stylesheet sized the stage, so the first version scaled every page's text to nothing. A stage with no size now means no change, and a `ResizeObserver` refits. Phones and anything narrower than 16:9 are untouched. `?frame=old` keeps the old behaviour for the owner's review |
| S47 | The pacer measures quietly (2026-09-26, final review) | An independent review of the branch found that on a device whose GPU cannot keep up, the time between animation frames is the GPU's, not the screen's, so the pacer took a 30 fps GPU for a 30 Hz screen and the governor judged every frame on time. The pacer now renders nothing while it measures (≈ 0.5 s at start and when the tab returns; the text keeps moving), and when frames run slow it checks for ≈ 0.33 s without rendering whether the screen slowed or the GPU did. Measured on the Intel laptop with tier 4 pinned: 60 Hz read correctly at 30 fps delivered. The governor restarts its window when its target changes; a resize no longer re-measures. The journey camera lands on the scroll on its first frame, so a deep link no longer swoops in from the open sea |
| S48 | The site matches the storyboard (2026-09-26) | The owner asked whether the build was truly the approved storyboard (canvas "Moonsink Shore — redesign directions", page Storyboard). Compared frame by frame, it was not. (1) **The sand was hidden**: one dark floor under the lower 62% of every page, 90% black at the bottom (S28's contrast fix), took ~78% of the light out of the foam and the black sand the camera lands on (measured 0.056 against 0.255 luminance). Each page now shades only where its text is, fading with its page: from the side on a laptop, from the bottom on a phone, with the storyboard's stops; the contrast tests now measure a laptop as well as a phone. (2) The project is the storyboard's card, with a "Read the case study" button. (3) The header lights up the page you are on. (4) Laptop margins as drawn: text 72 px in, the name block 236 px down, header and rail 48 px from the edges; type sizes from the frames. (5) The scroll hint sits at the foot of the screen: "Scroll to drift ashore · the moon fills as you go" on a laptop, "Swipe to drift ashore" on a phone; "Return to the shore · the moon wanes on the way back" on a laptop. (6) The phone header is one line: "22:10 · Crescent · 6%". (7) The case study is frame F: Year, Role and Stack, a captioned picture, the source as a button, and on a laptop the two tiles the storyboard left as placeholders, now real: six seconds of the journey rendered frame by frame on the GPU, and tier 0 against tier 3 side by side; the clip loops muted with its controls showing and plays by itself only without reduced motion. On a phone it fills the screen with the picture edge to edge. (8) The sand's glints are smaller and rarer (a soft point in one cell in 125, where a whole cell in 67 lit up), as the storyboard asked; the owner judges them. This deliberately sets aside §3.4's "no panels behind text" for the project card, which the storyboard draws |
| S49 | The owner's picks from S48, and one size for a window and full screen (2026-09-26) | Comparing S48 with the live site on a phone and a laptop, the owner kept: the header naming the page, the project card, the case study (frame F) and the smaller glints; and asked for the phone's case study to have the year, role and stack and the clip and comparison too (after the words). They moved "Scroll to drift ashore" to the foot of the screen, with no more words, and kept "Return to the shore" as it was. They set aside the rest of S48: the per-page shading (the dark floor of S28 stays), the storyboard's margins and sizes, the one-line phone header and the longer copy. **Window and full screen**: S46 kept the text in its place in the picture, so a window wider than 16:9 moved it inward and shrank it, while full screen drew it larger at the edge (owner's screenshots, 1897 × 886 against 1897 × 1080). Now a laptop (from 1024 px) draws every page at the normal window's 83% zoom from the edge, whatever the height, so a window and full screen of the same width match to the pixel; `?fit=inset` shows the alternative that keeps the window's 161 px margin, for the owner to pick. **Several projects**: a demo (`?demo`, three placeholders) lays a laptop out as the newest project's card with the rest as one-line rows, and a phone as a sideways row with a count; `?cards=list|row` shows the other layouts |
| S50 | The owner's final picks (2026-09-26) | (1) Several projects sit in the sideways row on every screen: on a laptop two whole cards side by side, each the storyboard's card with the picture beside the words, in a row kept within 48% of the screen so it stays clear of the moon, stepped through with ← and → and counted "1–2 / 4"; on a phone one card with the next peeking in. The row sits 2.2rem below "Projects" on a laptop, where the "j" had touched the card. The `?cards` switch and the card-and-rows layout are gone; `?demo` stays until real projects exist. (2) The laptop text starts 110 px in on the owner's 1897 px window (5.8% of the width), between the edge and the normal window's 161 px, "a bit to the left" of the inset they preferred; the `?fit` switch is gone. (3) The owner asked for "the lightly shining sand" back: the storyboard's per-page shading (S48) replaces the dark floor again |
| S51 | Last touches before the push (2026-09-26) | From the owner's phone and laptop: (1) Page IV's heading is in Fraunces italic (the name's face; Satoshi has no italic and a slanted one looks faked), 81.5 kB self-hosted; `?hello=2..4` shows "Let's talk", "Write to me" and "Send word" until the owner picks. (2) On a phone page IV sits nearer the foot (6% up, was 12%), which had left an empty band under "Return to the shore". (3) On a phone the log's parts no longer break mid-phrase: "WAXING CRESCENT · 27% LIT" had wrapped and put "LIT" on the rail's first moon; letter-spacing 0.1em keeps it beside "Email me" at 360 px (tested at every phase). (4) The sand shines: it was about 1% grey, so even with no text over it the phone's foot of the picture read as black. It now has a base the moon lifts a little, a soft moonlit sheen on the side facing the moon, and glints in one cell of 83 (was 125); on a phone page IV's shading is lighter at the very foot, where only the links cross the sand. Every contrast test still passes at full moon on a phone and a laptop |
| S52 | The owner's picks before the push (2026-09-26) | (1) Page IV's heading is "Over to you", chosen from four wordings; the `?hello` switch is gone. (2) The Projects row holds Moonlit and two "Coming soon" cards ("In the works", a still for a picture, no case study and no button), on a phone and a laptop: the owner asked for several cards live, and placeholder projects with made-up case studies would read as unfinished to a visitor. A real project replaces one in `content/projects.ts` (`upcoming`). The `?demo` placeholders are gone. (3) The sand on a phone: at a phone's tier the lower third of page IV, exactly where the links sit, was under 60–94% black. Page IV's phone shading is now light at the foot (25–40%), dark only in a band behind the chapter title and the heading (they cross the foam), and clear above, so the moon's path shows; page III's foot is lighter too, the cards being solid. The contrast helper now measures only text on screen: a card scrolled out of the row had been measured against the scene behind its hidden spot. All eight contrast tests pass |
