...
test('?length sets how far the journey scrolls, and the default is 2.7 screen heights', async ({ page }) => {
  await page.goto('/?stills&length=3');
  await expect(page.locator('#journey-track')).toHaveAttribute('style', /--journey-length:\s*3\b/);
  await page.goto('/?stills');
  await expect(page.locator('#journey-track')).toHaveAttribute('style', /--journey-length:\s*2\.7\b/);
});
